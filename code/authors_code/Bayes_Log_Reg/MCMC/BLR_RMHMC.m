...
for IterationNum = 1:NumOfIterations
    ...
    for StepNum = 1:RandomSteps
        ...
        for FixedIter = 1:NumOfNewtonSteps
            ...
            Pw = wNew + (TimeStep*(StepSize/2))*OriginalInvGMomentum + (TimeStep*(StepSize/2))*InvGMomentum;
        end
        wNew = Pw;
        ...
        dHdTheta = -( XX'*( t - (1./(1+exp(-f))) ) - eye(D)*(1/alpha)*wNew - 0.5*TraceInvGdG' + LastTerm' );
    ...
    ProposedLogDet = 0.5*( log(2) + D*log(pi) + 2*sum(log(diag(chol(G)))) );
...
CurTime = fix(clock);
cd('..')
save(['Results/Results_RMHMC_' num2str(NumOfLeapFrogSteps) '_' num2str(NumOfNewtonSteps) '_' DataSet '_' num2str(floor(now)) '_' num2str(CurTime(4:6)) '.mat'], 'betaPosterior', 'TimeTaken')
